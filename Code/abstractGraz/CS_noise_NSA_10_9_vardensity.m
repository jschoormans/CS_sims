...
P=struct;
P.chan=1;
P.folder='/home/jschoormans/lood_storage/divi/Projects/cosart/scans/10-9-grapefruit-CS-Graz/'
cd(P.folder)
files=dir('*.raw');
P.file=files(14).name;
...
ImRef=fftshift(squeeze(bart(['pics -RW:7:0:',num2str(reg),' -S -e -i500'],double(Kref),sens)),2); %to compare image
ImRef=abs(ImRef);
