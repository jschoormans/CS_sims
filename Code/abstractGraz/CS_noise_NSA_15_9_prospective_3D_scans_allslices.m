...
cd('L:\basic\divi\Projects\cosart')
addpath(genpath('CS_simulations'))
...
data=sum(Ks,3);   %mean of data over NSA
data=data./MNSA;
data(isnan(data))=0
...
% L1 Recon Parameters 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
TVWeight = 0.0002; 	% Weight for TV penalty
xfmWeight = 0.000;	% Weight for Transform L1 penalty
...
%% COMBINE CHANNELS
Rfull(:,:,sl)=squeeze(sqrt(sum(R.^2,3)));
if sl==1
save(['R-sl1-',num2str(filenumber),'.mat'],'Rfull')
else
save(['R-sl1-',num2str(filenumber),'.mat'],'Rfull','-append')
end
end