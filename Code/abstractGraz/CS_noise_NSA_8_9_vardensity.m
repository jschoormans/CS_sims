...
accvector=[7]
% accvector=[1,4,8]
noisevector=[2] % in this case the different W 
...
xfmWeight = 0.005;	% Weight for Transform L1 penalty %0.001
...
for ii=1:length(accvector);
    for jj=1:length(noisevector)
        ...
        coordsx=590; coordsy=870:900;
        Edge=abs(squeeze(R{2,ii,jj}(coordsx,coordsy)));
        Center=21; %whatever
        Sigmoid(2,ii,jj)=SigmoidFitting(Edge,Center,1,1);
