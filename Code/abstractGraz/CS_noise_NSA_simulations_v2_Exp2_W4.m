...
if false
Im=imread('head512.jpg');   %image in image domain
Im=double(Im(:,:,1))./255;
K=bart('fft 3',Im);
K=K(129:387,129:387)
sens=ones(size(K));
...
figure(105); count=1;
for ii=1:length(accvector); for jj=1:length(no isevector); subplot(length(accvector),length(noisevector),count); count=count+1;
imshow(abs(R{2,ii,jj}([150:200],[180:230])),[]); axis off; end; end; tightfig; 
