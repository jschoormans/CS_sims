function [Recon,D,D2,D3,acc,Msave,MSV]=analyse_CSSNR(Im,K,sens,NoiseLevels,regs,unders,iters,restrictellipse)
...
for iter=1:iters
    ...
    for kk=1:length(unders)
        ...
        [pdf,val] = genPDF(size(K),5,unders(kk),2,0.05,0);
        Mfull=genEllipse(size(K,1),size(K,2));
        
        
        if  ~exist('restrictellipse')
            M(:,:,kk)=genSampling(pdf,10,0.5).*Mfull;
        elseif restrictellipse(iter)==1
            M(:,:,kk)=genSampling(pdf,10,0.5).*Mfull;
        else
            M(:,:,kk)=genSampling(pdf,10,0.5)
        end
        Msave(:,:,:,iter)=M(:,:,:);
...
Recon(:,:,1,jj,kk,ll,iter)=bart(['pics -RW:6:0:',num2str(reg),' -e -i100'],Ku_N,sens);
Recon(:,:,2,jj,kk,ll,iter)=bart(['pics -RW:6:0:',num2str(reg),' -e -i100'],K_N,sens);
...
maR1=max(abs(squeeze(Recon(:,:,1,jj,kk,ll,iter))));
maR2=max(abs(squeeze(Recon(:,:,2,jj,kk,ll,iter))));


D2(1,jj,kk,ll,iter)=ssim(abs(squeeze(Recon(:,:,1,jj,kk,ll,iter)))./max(maR1(:)),abs(Im)');
D2(2,jj,kk,ll,iter)=ssim(abs(squeeze(Recon(:,:,2,jj,kk,ll,iter)))./max(maR2(:)),abs(Im)');
