function x = fnlCg_test(x0,params)
%-----------------------------------------------------------------------
%
% res = fnlCg(x0,params)
%
...
% backtracking line-search
	...
	while (f1 > f0 - alpha*t*abs(g0(:)'*dx(:)))^2 & (lsiter<maxlsiter) %FIND T SUCH THAT OBJECTIVE FUNCTION IS NOT TOO BAD
		lsiter = lsiter + 1;
		t = t * beta;
		[f1, ERRobj, RMSerr]  =  objective(FTXFMtx, FTXFMtdx, DXFMtx, DXFMtdx,x,dx, t, params);
	end
...
if params.Debug==1;
   ...
   figure(1); hold on; plot(real(Errors(257,:)),'r'); plot(real(ErrorsW(257,:)),'k'); hold off;
