...
for n=1:iter
	...
	while abs(sum(tmp(:)) - K) > tol
		tmp = rand(size(pdf))<pdf;      %generates the sampling pattern with as manuy zeroes (within tol) as integrated probability
    end
	TMP = ifft(tmp./pdf);               %finds ift of tmp/pdf ??
	if max(abs(TMP(2:end))) < minIntr   %probably to filter strange results >1e99
