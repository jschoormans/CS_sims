...
if true %make example image more sparse in W-domainby soft-thresholding
wavcoeff=bart('cdf97 7', Im); %wavelet coefficietns;

sparsity_pre_th=sum(wavcoeff(:)~=0)/(ny*nz*nx);

wavcoeffz1=wavcoeff(1:64,1:64,:);

Imr2=bart('cdf97 7 -i', wavcoeffz1(:,:,:));

wavcoeff2=bart('cdf97 7', Imr2); %wavelet coefficietns;



% wavcoeff= bart('threshold 4',wavcoeff);

Im=bart('cdf97 7 -i', wavcoeff);
as(Im)
end
sparsity=sum(wavcoeff(:)~=0)/(ny*nz*nx)
...
Ku=permute(Ku,[3 2 1]);
%CS
ImCS6=bart('pics -RW:7:0:0.01 -i100',Ku,sens);
...
figfolder=['/home/jschoormans/lood_storage/divi/Projects/cosart/CS_simulations/Figures/','3d-',num2str(experiment_number)]
cd(figfolder)
