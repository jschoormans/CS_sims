...
[kspace, trajfull]=calctrajBART(permute(Ku_N2,[1 2 3 4 6 5])); 
...
traj=trajfull;
R2{jjj}=bart(['pics -RW:7:0:',num2str(reg),' -S  -e -i100 -t'],traj,kspace,sens);
...
traj=traj.*(ny/2);
R3{jjj}=bart(['pics -RW:7:0:',num2str(reg),' -S  -e -i100 -t'],traj,kspace,sens);
