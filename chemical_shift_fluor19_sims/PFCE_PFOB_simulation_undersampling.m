...
CGK=nl_conjgrad_fluor(A2,K,zeros(64^2,1),40,I(:),3e-1,64,64); 
...
figure(5); imshow(rr(abs(opInverse(A)*(R'*K))),[minschaal maxschaal]); axis off;
export_fig deconvolution.pdf -native
figure(6); imshow(rr(abs(CGK)),[minschaal maxschaal]); axis off;
