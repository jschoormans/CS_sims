%   modification of mrics.m  by Tom Goldstein (TomGoldstein1@gmail.com)
...
function u = mricsmodwaveletTV(R,f, mu, lambda, gamma,delta, nInner, nBreg)
    [rows,cols] = size(f);
    %W=Wavelet;   
    ...
    uker = mu*(conj(R).*R)+lambda*fft2(uker)+delta+gamma;
    ...
    for outer = 1:nBreg;
        for inner = 1:nInner;
            ...
            %rhs = murf+lambda*Dxt(x-bx)+lambda*Dyt(y-by) +gamma*(W'*(w-bw)); 
            %rhs = murf+lambda*Dxt(x-bx)+lambda*Dyt(y-by) +gamma*(ifwt2((w-bw),'db4',4));
            %rhs = murf+delta*(ifwt2((w-bw),'db4',4))+gamma*u;
            rhs = murf+lambda*Dxt(x-bx)+lambda*Dyt(y-by)+delta*(ifwt2((w-bw),'db4',4))+gamma*u;

            u = ifft2(fft2(rhs)./uker); %where is the kernel good for?
